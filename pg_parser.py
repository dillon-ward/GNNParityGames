...
def parse_regions(region_str):
    try:
        return np.array(re.match(r'\{(.*)\}', region_str).group(1).split(',')).astype(np.int32)
    except:
        return np.empty(0, dtype=np.int32)
        
def parse_strategy(strategy_str):
    try:
        return np.array([x.split('->') for x in re.match(r'\[(.*)\]', strategy_str).group(1).split(',')]).astype(np.int32)
    except:
